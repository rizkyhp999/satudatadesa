...
export default function T3P3({ data }: Props) {
  ...
  const summary = useMemo(() => {
    ...
    data.anggota
      .filter((a: any) => {
        ...
        return !isNaN(umur) && umur >= 7 && umur <= 23;
      })
      .forEach((a: any) => {
  ...
  const handleDownloadExcel = () => {
    ...
    saveAs(blob, "penduduk_7_23_status_pendidikan.xlsx");
  };
  ...
  const handleDownloadChart = async () => {
    ...
    try {
      ...
      link.download = "grafik_pendidikan_umur7_23.png";
      link.href = dataUrl;
      link.click();
    } catch (err) {
    ...
    <div>
      <div className="flex flex-col gap-6 md:flex-row md:items-start">
        <Card className="mb-6 md:mb-0 md:w-1/2 flex flex-col">
          <CardHeader>
            <CardTitle>
              Jumlah Penduduk Berumur 7-23 Tahun ke Atas Menurut Jenis Kelamin
              dan Status Pendidikan di Desa Kapuak, 2025 (Grafik)
            </CardTitle>
          </CardHeader>
        ...
        <Card className="md:w-1/2 flex flex-col">
          <CardHeader>
            <CardTitle>
              Jumlah Penduduk Berumur 7-23 Tahun ke Atas Menurut Jenis Kelamin
              dan Status Pendidikan di Desa Kapuak, 2025 (Tabel)
            </CardTitle>
          </CardHeader>
          <CardContent className="flex-1 flex flex-col">
