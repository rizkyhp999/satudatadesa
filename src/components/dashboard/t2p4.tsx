...
export default function T2P4({ data }: { data?: any }) {
  const anggota = data?.anggota || [];
  const chartRef = useRef<HTMLDivElement>(null);

  // loading dan error dihandle di page.tsx

  const rows = useMemo(() => {
    if (!anggota || anggota.length === 0) return [];
    // Filter hanya yang bekerja
    const bekerja = anggota.filter(
      (a: any) =>
        a["501_bekerjaMingguLalu"] === "1" && a["502_lapanganUsahaUtama"]
    );
    // Group by lapangan usaha dan jenis kelamin
    const result: Record<string, { laki: number; perempuan: number }> = {};
    bekerja.forEach((a: any) => {
      const kodeUsaha = a["502_lapanganUsahaUtama"];
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Grafik Jumlah Penduduk Bekerja per Lapangan Usaha",
      },
    },
    scales: {
      x: { stacked: true },
  ...
  const handleDownloadChart = async () => {
    ...
    try {
      ...
      link.download = "grafik_t2p4_lapangan_usaha.png";
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleDownloadTable = () => {
    ...
    saveAs(blob, "rekap_lapangan_usaha_jenis_kelamin.xlsx");
  };
      ...
      <Card className="mb-6 md:mb-0 md:w-1/2 flex flex-col border border-gray-200">
        <CardHeader className="bg-white border-b border-gray-200">
          <CardTitle className="text-lg font-semibold">
            Tabel 2.4 Jumlah Penduduk yang Bekerja Menurut Lapangan Usaha dan
